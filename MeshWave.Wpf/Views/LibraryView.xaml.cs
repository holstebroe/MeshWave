...
namespace MeshWave.Wpf.Views
{
    public partial class LibraryView : UserControl
    {
        ...
        private void Vm_OpenMetadataEditorRequested(object? sender, string trackFilePath)
        {
            ...
            if (Application.Current.MainWindow.DataContext is ApplicationViewModel appVm)
            {
        ...
        {
            if (sender is not ListBox listBox || listBox.SelectedItem is not LibraryTrackItem trackItem)
            {
                return;
            }
        ...
        {
            if (sender is not ListBox listBox || listBox.SelectedItem is not LibraryAlbumItem albumItem)
            {
                return;
            }

            if (DataContext is not LibraryViewModel vm || !vm.CanImportMyMusic)
            {
                return;
            }
        ...
        private void ImportMyMusicFile_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == true && DataContext is LibraryViewModel vm)
            {
