...
public class AudioPlaybackService : IAudioPlaybackService
{
    ...
    public async Task LoadGrowingFileAsync(string tempPath, long totalBytes, CancellationToken ct = default)
    {
        ...
        while (new FileInfo(tempPath).Length < Math.Min(initialBufferBytes, totalBytes) && !ct.IsCancellationRequested) await Task.Delay(500, ct);
    ...
    public void Play()
    {
        ...
        if (_waveOut == null && !string.IsNullOrEmpty(_currentFilePath)) LoadFile(_currentFilePath);
...
internal class GrowingFileStream : Stream
{
    ...
    public override long Seek(long offset, SeekOrigin origin)
    {
        switch (origin)
        {
            ...
            case SeekOrigin.End: _position = TotalLength + offset; break;
        }
        return _position;
    }
