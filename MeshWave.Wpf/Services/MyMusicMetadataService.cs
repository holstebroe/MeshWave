...
public class MyMusicMetadataService
{
    ...
    {
        if (string.IsNullOrWhiteSpace(filePath) || !File.Exists(filePath) || !File.Exists(sourceImagePath)) return;
    ...
    {
        if (string.IsNullOrWhiteSpace(albumFolder) || !Directory.Exists(albumFolder)) return new MyMusicMetadata();
        ...
        if (firstTrack == null) return new MyMusicMetadata { Title = Path.GetFileName(albumFolder) ?? "Unknown Album" };
    ...
    public string GetCoverArtPath(string filePath)
    {
        ...
        try
        {
            ...
            var picture = tagFile.Tag.Pictures?.FirstOrDefault(p => p.Type == PictureType.FrontCover)
    ...
    {
        if (string.IsNullOrWhiteSpace(albumFolder) || !Directory.Exists(albumFolder)) return string.Empty;
        ...
        foreach (var name in imageFiles)
        {
            ...
            if (File.Exists(path))
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(cacheDir)) Directory.CreateDirectory(cacheDir);
                    if (!File.Exists(cachePath) || File.GetLastWriteTimeUtc(path) > File.GetLastWriteTimeUtc(cachePath)) File.Copy(path, cachePath, true);
                    return cachePath;
                }
                catch { }
            }
    ...
    {
        if (string.IsNullOrWhiteSpace(filePath) || !File.Exists(filePath)) return new MyMusicMetadata();
    ...
    {
        if (string.IsNullOrWhiteSpace(albumFolder)) return string.Empty;
    ...
    public byte[]? CreateIcon(string imagePath)
    {
        ...
        try
        {
            ...
            var scale = targetSize / Math.Max(frame.PixelWidth, frame.PixelHeight);
