...
public class SettingsService
{
    ...
    public virtual AppSettings LoadSettings()
    {
        ...
        if (_currentSettings.SupportedExtensions == null || _currentSettings.SupportedExtensions.Count == 0) _currentSettings.SupportedExtensions = [".mp3", ".flac", ".wav", ".ogg", ".m4a"];
    ...
    public void EnsureFoldersExist()
    {
        ...
        if (!Directory.Exists(settings.BaseFolder)) Directory.CreateDirectory(settings.BaseFolder);

        var localMusic = GetLocalMusicFolder();
        if (!Directory.Exists(localMusic)) Directory.CreateDirectory(localMusic);

        var peerMusic = GetPeerMusicFolder();
        if (!Directory.Exists(peerMusic)) Directory.CreateDirectory(peerMusic);
    }
